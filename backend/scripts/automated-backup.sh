...
# Enhanced Database Backup Script with Configuration File Support
# This script creates daily backups of both MySQL and MongoDB databases

# Load configuration file if it exists
CONFIG_FILE="$(dirname "$0")/backup-config.env"
...
MYSQL_DATABASE="${MYSQL_DATABASE:-${DB_NAME:-ecommerce_db}}"
...
MONGO_URI="${MONGO_URI:-${DB_CONNECTION_STRING:-mongodb://localhost:27017}}"
...
# Remove MySQL backups older than RETENTION_DAYS days
find "$BACKUP_DIR/mysql" -name "mysql_backup_*.sql.gz" -mtime +$RETENTION_DAYS -delete 2>>"$LOG_FILE"
log "Old MySQL backups cleaned up"

# Remove MongoDB backups older than RETENTION_DAYS days
find "$BACKUP_DIR/mongodb" -name "backup_*.tar.gz" -mtime +$RETENTION_DAYS -delete 2>>"$LOG_FILE"
